...
public class Exam07 {
	...
	public static void main(String[] args) {
		...
		while (run) {
			...
			} else if (selectNo == 2) { // 1~45 중에 랜덤한 값을 1개씩 가져옴
				if (count < numList.length) {
					int num = (int) (Math.random() * 45) + 1;
					numList[count++] = num; // 인덱스 역할도 하고 몇번 카운트됐는지도 알 수 있음
					System.out.println("현재 가져온 수의 갯수 : " + count);
				} else {
					System.out.println("더이상 숫자를 가져올 수 없습니다");
				}
			} else if (selectNo == 3) { // 기존의 값 중 가장 최근 값을 1개 삭제\
			//count = 6 -> index = count -1 
				numList[--count] = 0; //--뒤로 가면 절대 안됨XXXXXXXX
				System.out.println("현재 가져온 수의 갯수 : " + count);
			...
			} else if (selectNo == 5) { // 중복 유무와 중복된 값이 몇번 중복되었는지
				int[]list = new int[45];
				//개표
				for(int i=0; i < count; i++) {
